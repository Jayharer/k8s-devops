...
resource "aws_vpc_security_group_ingress_rule" "allow-rule1" {
  security_group_id = aws_security_group.ec2-sg.id
  cidr_ipv4         = "0.0.0.0/0"
  ip_protocol       = "-1"
}

resource "aws_vpc_security_group_ingress_rule" "allow-rule2" {
  security_group_id = aws_security_group.ec2-sg.id
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = 8472
  to_port           = 8472
...
resource "aws_vpc_security_group_ingress_rule" "allow-rule3" {
  security_group_id = aws_security_group.ec2-sg.id
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = 10250
  to_port           = 10250
